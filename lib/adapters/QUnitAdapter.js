...
export default class QUnitAdapter extends EventEmitter {
  ...
  createGlobalSuite () {
    ...
    let suites = modules.map(this.convertModule)
  ...
  createTestEnd (test) {
    const {status, runtime, errors} = this.testsDetails[test.testId]

    return new Test(test.testName, test.suiteName, status, runtime, errors)
  }
