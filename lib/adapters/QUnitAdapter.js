...
module.exports = class QUnitAdapter extends EventEmitter {
  ...
  convertModule (qunitModule) {
    ...
    return new SuiteStart(
      ...
      qunitModule.tests.map((qunitTest) => {
        ...
        const suiteName = qunitModule.name.substring(indexStart);
        const test = new TestEnd(qunitTest.name, suiteName, fullName.slice());
  ...
  saveTestDetails (qunitTest) {
    const test = this.tests[qunitTest.testId];
  ...
  createGlobalSuite () {
    ...
    const suites = modules.map(this.convertModule.bind(this));
    ...
    suites.forEach((suite) => {
      const indexEnd = suite.name.lastIndexOf(this.delim);
      ...
      if (indexEnd !== -1) {
        ...
        const parentSuiteName = suite.name.substring(indexStart, indexEnd);
